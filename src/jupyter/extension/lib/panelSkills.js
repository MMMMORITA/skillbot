...
exports.enterSkillsMode = enterSkillsMode;
exports.exitSkillsMode = exitSkillsMode;
exports.renderSkillList = renderSkillList;
exports.promptUploadSkill = promptUploadSkill;
exports.restartAgent = restartAgent;
exports.onSkillKeydown = onSkillKeydown;
exports.refreshSkillRows = refreshSkillRows;
exports.toggleSkill = toggleSkill;
exports.uninstallSkill = uninstallSkill;
exports.updateSkillHint = updateSkillHint;
exports.showSkillNotice = showSkillNotice;
exports.renderSkillInfo = renderSkillInfo;
const panelStyles_1 = require("./panelStyles");
function enterSkillsMode(panel) {
    panel._skillsMode = true;
    panel._inputWrapper.style.display = 'none';
    panel._outputEl.querySelectorAll('.skillbot-skill-list').forEach((el) => el.remove());
    panel._renderActionBar();
}
function exitSkillsMode(panel) {
...
function renderSkillList(panel, skills) {
...
function promptUploadSkill(panel) {
...
function onSkillKeydown(panel, e) {
    ...
    if (panel._fullBodyIdx !== -1) {
        if (e.key === 'Escape') {
            ...
            panel._fullBodyIdx = -1;
            refreshSkillRows(panel);
            setTimeout(() => { var _a; return (_a = panel._skillListWrapper) === null || _a === void 0 ? void 0 : _a.focus(); }, 0);
        }
        return;
    }
...
function refreshSkillRows(panel) {
    var _a;
    const listEl = (_a = panel._skillListWrapper) === null || _a === void 0 ? void 0 : _a.querySelector('.skillbot-skill-items');
    if (!listEl)
        return;
    listEl.innerHTML = '';
    panel._skillRows = [];
    ...
    panel._skillData.forEach((s, idx) => {
        if (!matches(s))
            return;
    ...
    for (const cat of cats) {
        ...
        catHeader.innerHTML =
            `<span style="color:rgb(150,150,150);font-size:10px;width:10px;display:inline-block;">${chevron}</span>` +
                `<span style="color:${panelStyles_1.CC.text};font-size:11px;font-weight:600;">${panel._esc(cat)}</span>` +
                `<span style="color:rgb(130,130,130);font-size:10px;margin-left:auto;">${enabledCount}/${items.length}</span>`;
        catHeader.addEventListener('click', () => {
            if (panel._collapsedCats.has(cat))
                panel._collapsedCats.delete(cat);
            else
                panel._collapsedCats.add(cat);
            refreshSkillRows(panel);
        });
        ...
        for (const { s, idx } of items) {
            ...
            if (expanded) {
                ...
                if (s.body) {
                    ...
                    detail.innerHTML += `<div style="color:${panelStyles_1.CC.text};background:rgba(255,255,255,0.03);padding:6px;border-radius:3px;max-height:${maxH}px;overflow-y:auto;white-space:pre-wrap;font-size:11px;">${panel._esc(bodyText)}${(!showFull && s.body.length > 1000) ? '…' : ''}</div>`;
...
function toggleSkill(panel, idx) {
    var _a;
    ...
    const btn = (_a = panel._skillListWrapper) === null || _a === void 0 ? void 0 : _a.querySelector('.skillbot-skill-restart');
    if (btn)
        btn.classList.add('pending');
}
/** Uninstall a skill after an inline confirm click. */
function uninstallSkill(panel, idx) {
    const s = panel._skillData[idx];
    if (!s || !panel._kernel)
        return;
...
function updateSkillHint(panel) {
    var _a;
    const hintEl = (_a = panel._skillListWrapper) === null || _a === void 0 ? void 0 : _a.querySelector('.skillbot-skill-hint');
    if (!hintEl)
        return;
    if (panel._skillData.length === 0) {
...
function showSkillNotice(panel, msg) {
    if (!msg || !panel._skillListWrapper)
        return;
    let el = panel._skillListWrapper.querySelector('.skillbot-skill-notice');
    if (!el) {
    ...
    el.textContent = msg;
    if (panel._skillNoticeTimer)
        clearTimeout(panel._skillNoticeTimer);
    panel._skillNoticeTimer = setTimeout(() => { if (el)
        el.textContent = ''; }, 6000);
}
function renderSkillInfo(panel, skill) {
    const wrapper = document.createElement('div');
