...
function onSkillKeydown(panel, e) {
    ...
    switch (e.key) {
        case 'i':
            ...
            setTimeout(() => {
                var _a;
                const inp = (_a = panel._skillListWrapper) === null || _a === void 0 ? void 0 : _a.querySelector('.skillbot-install-input');
        ...
        case 'd':
            ...
            const cancelTimer = setTimeout(() => {
                var _a;
                cancelled = true;
                (_a = panel._skillListWrapper) === null || _a === void 0 ? void 0 : _a.removeEventListener('keydown', onConfirm);
                if (hintEl) {
                    hintEl.style.color = '';
                }
                refreshSkillRows(panel);
            ...
            setTimeout(() => {
                var _a;
                (_a = panel._skillListWrapper) === null || _a === void 0 ? void 0 : _a.addEventListener('keydown', onConfirm, { once: false });
            }, 0);
            break;
        case 'Tab':
        case 'ArrowDown':
...
function refreshSkillRows(panel) {
    ...
    panel._skillData.forEach((s, i) => {
        ...
        header.innerHTML = `${dot} <span style="color:${panelStyles_1.CC.text};font-size:12px;">${panel._esc(s.name)}</span> <span style="color:${statusColor};font-size:10px;margin-left:auto;">${status}</span>`;
        ...
        if (expanded) {
            ...
            if (showFull) {
                ...
                detail.innerHTML = `<div style="color:${panelStyles_1.CC.text};background:rgba(255,255,255,0.04);padding:8px;border-radius:3px;max-height:350px;overflow-y:auto;white-space:pre-wrap;font-size:11px;line-height:1.4;">${panel._esc(s.body || '')}</div>`;
            }
            else {
                ...
                if (s.body) {
                    ...
                    detail.innerHTML += `<div style="color:${panelStyles_1.CC.text};background:rgba(255,255,255,0.03);padding:6px;border-radius:3px;max-height:150px;overflow-y:auto;white-space:pre-wrap;font-size:11px;">${panel._esc(bodyText)}${s.body.length > 1000 ? '...' : ''}</div>`;
                }
            }
    ...
    const hintEl = (_b = panel._skillListWrapper) === null || _b === void 0 ? void 0 : _b.querySelector('.skillbot-skill-hint');
    if (hintEl) {
        if (panel._installMode) {
