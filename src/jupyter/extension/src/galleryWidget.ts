...
import { GALLERY_STYLES } from './galleryStyles';
...
export class SkillsGalleryWidget extends Widget {
  private _root: ShadowRoot;
  private _bodyEl!: HTMLElement;
  private _skillsPage!: HTMLElement;
  private _kbPage!: HTMLElement;
  private _tabSkills!: HTMLElement;
  private _tabKb!: HTMLElement;
  private _kernel: any = null;
  private _comm: any = null;
  private _skills: SkillItem[] = [];
  private _kbDocs: KbDoc[] = [];
  private _pendingDoc: KbDoc | null = null;
   ...
   */
  onDocOpen: ((doc: KbDoc, html: string) => void) | null = null;
  ...
  connectKernel(kernel: any): void {
    ...
    try {
      ...
      this._comm.onMsg = (m: any) => this._onMsg(m.content?.data || {});
      this._comm.open();
      this.refresh();
    } catch (e) {
    ...
    switch (d.action) {
      ...
      case 'gallery_kb_html':
        if (this._pendingDoc && typeof d.html === 'string') {
          this.onDocOpen?.(this._pendingDoc, d.html);
  ...
  private _openDoc(doc: KbDoc): void {
    if (!this._kernel) return;
  ...
  private _switchTab(tab: 'skills' | 'kb'): void {
    const on = tab === 'skills';
  ...
  private _renderEmpty(page: HTMLElement, msg: string): void {
    page.innerHTML = '';
  ...
  private _renderKb(): void {
    ...
    const seen: string[] = [];
    const byCat: Record<string, KbDoc[]> = {};
    for (const d of this._kbDocs) {
      const cat = d.category || 'Uncategorized';
      if (!byCat[cat]) { byCat[cat] = []; seen.push(cat); }
      byCat[cat].push(d);
    }
  ...
  private _kbCard(doc: KbDoc): HTMLElement {
    const card = document.createElement('div');
