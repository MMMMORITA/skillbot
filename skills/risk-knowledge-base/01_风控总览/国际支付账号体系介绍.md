---
title: 国际支付账号体系介绍 GP Account System Introduction（2024.06）
category: 风控总览
source_url: https://bytedance.sg.larkoffice.com/wiki/SOBvwqWBwiUhSBkEmdZc8PECnVr
source_token: SOBvwqWBwiUhSBkEmdZc8PECnVr
source_type: wiki
doc_type: 业务介绍
tags: [账号体系, 账号安全, 指标, PIPO, GPP, 电商, 直播]
metrics: [支付账号开户转化率, 月活用户身份信息覆盖率, TikTok卡包渗透（PI合并进度）, TikTok KYC合并进度]
business_lines: [电商, 直播]
summary: 介绍国际支付(GP)账号体系模型、标识定义、功能现状与迭代规划
last_synced: 2026-08-12
---

## 适用场景
需要理清国际支付（GP/PIPO）账号体系现状、各类账号标识含义及关系，或避免日常工作中账号概念误用时参考。适用于理解 PIPOuid/Wuid/Muid/TTuid/PI/客户号/商户号等标识的定义与生成逻辑、账号功能（注册/管理/PI 互通/删除）现状，以及 C 端/B 端账号模型的迭代方向与团队协作分工。

## 核心概念
- 账号方向 = 会员域 ≠ CA 账户；包括用户 ID、用户信息（资料/验证方式/KYC）、用户资产关系（PI、账户关系），不包括 KYC 核验能力、PI 详情、CA 账户详情。
- 字节账号体系：多数业务使用中台统一 Passport 账号（统一发号器）；国际支付、飞书、People、火山引擎自建账号能力。aid 标识独立端，账号组（agid）决定不同端账号是否互通。
- 平台号（Platform ID）：关联属于不同主体但共用同一 User 数据源的多个商户（仅 PIPO 内）。
- 客户号（Client ID）：商户入驻提交 KYC 后生成的客户唯一标识（自然人/法人）。
- 商户号（MID）：客户实际开展支付业务的唯一标识，分一级/二级商户号。
- 商户用户（MUID）：业务侧用户 id；type=user 时为真实 uid，type=merchant 时 merchant_uid=PIPO uid=client_id。
- PIPO uid：PIPO 系统唯一识别用户标识；C 类 = generate(Platform id + Muid)，B 类直接用传入 muid；与 TTuid 不对等。
- 统一账号 Wuid（计划改名 PAuid）：对齐 TTuid 粒度的支付用户唯一标识，解决一个 TTuid 被切成多个 PIPOuid 导致 PI/KYC 资产不互通问题，为 PIPOuid 的合并升级版。
- 支付工具 PI：用户资产唯一标识；PI ID 由 UserID + Payment Method + PI Identity（卡号/银行账号等）决定；现状 UserID 有 PIPOuid 与 Wuid（PI Clip）两套逻辑。
- 三户模型：客户（社会唯一身份，存实名信息）、用户（产品使用者）、账户（用钱的身份，存资金/债权/收益）。

## 关键指标口径
账号核心指标（现状值在原文以 * 脱敏）：
- 支付账号开户转化率 = 完成手机号绑定和密码设置的用户 / 钱包、BNPL 涉及的用户。
- 月活用户身份信息覆盖率 = 至少拥有一种可验身份信息的用户 / TikTok Pay 用户。
- TikTok 卡包渗透（PI 合并进度）= TikTok 体系中已上线 PI Clip 的流量 / 所有流量。
- TikTok KYC 合并进度 = TikTok 体系中已上线统一 KYC 的流量 / 所有流量。

## 规则 / 策略要点
- 账号标识切换节奏：PIPOuid → Wuid，先 C 端（24 年）再 B 端（25 年）；C 端先 TT 再其他；TT 中先 TTS 和 TTL（预计 24 年内基本完成）再其他（UG 等），24~25 年 PIPOuid 与 Wuid 双轨并行。
- 注册信息收集差异（按场景）：Creator Wallet 全量强制收集（手机号+OTP、Pin、KYC）；BNPL 按国家政策收集（手机号+OTP、Pin、KYC）；Consumer Wallet-Close loop 仅对高风险用户（约 10%）收集 Pin；Consumer Wallet-Open loop 收集手机号+OTP、Pin，onboarding 后引导 KYC；Merchant Wallet 收集手机号+OTP、Pin。
- PI 互通：23 年底前支付工具仅限本业务线内使用；24 年 1-9 月在 TTS、TTL 逐步放量实现跨业务线互通。
- 账号删除（计划改为「关闭支付功能」）：TT 账号注销需先处理支付账号资产并同步清除支付数据；ID 钱包申牌场景需提供独立钱包注销能力。
