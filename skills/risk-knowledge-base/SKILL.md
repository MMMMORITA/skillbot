...
## 会话开始必做（orientation）
1. 读 `SCHEMA.md` 了解 10 类目定义与 tags 受控词表。
2. 读 `index.md` 了解现有文档及一句话摘要。
3. 读 `log.md` 最近若干条了解最新变更。

## 检索流程
1. 从任务中抽取：**类目 / 指标名 / 业务线 / 关键词**。
2. 读 `manifest.json`，按 `category` / `metrics` / `business_lines` / `tags` 过滤命中文档（无需逐个打开 MD）。
3. 打开命中 MD，优先读「关键指标口径」「规则 / 策略要点」节；口径细节与完整 SQL 在「原文正文」节。
4. 将命中的口径定义、规则要点作为参考注入主流程，并**标注来源 MD 与 source_url**。
